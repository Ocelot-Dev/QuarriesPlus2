...
public class BlockMarkerPlus extends BlockContainerBase {

	public BlockMarkerPlus(int i) {
	...
	public TileEntity createNewTileEntity(World var1) {
		return new TileMarkerPlus();
	}

	@Override
	public void onNeighborBlockChange(World world, int x, int y, int z, int blockId) {
		((TileMarkerPlus) world.getBlockTileEntity(x, y, z)).G_updateSignal();
		dropTorchIfCantStay(world, x, y, z);
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer ep, int par6, float par7, float par8, float par9) {
		if (!world.isRemote) {
			...
			if (equipped instanceof ItemTool && ep.getCurrentEquippedItem().getItemDamage() == 0) {
				TileMarkerPlus.Link l = ((TileMarkerPlus) world.getBlockTileEntity(x, y, z)).link;
			...
			((TileMarkerPlus) world.getBlockTileEntity(x, y, z)).S_tryConnection();
		}
		return true;
	}
	...
	public void onPostBlockPlaced(World world, int x, int y, int z, int meta) {
		((TileMarkerPlus) world.getBlockTileEntity(x, y, z)).requestTicket();
	...
	public void registerIcons(IconRegister par1IconRegister) {
		this.blockIcon = par1IconRegister.registerIcon("qp2:marker");
	}
}

...
public class BlockPumpPlus extends BlockContainerBase {

	private Icon textureTop, textureBottom, textureSide, texW, texC;

	public BlockPumpPlus(int i) {
	...
	public Icon getBlockTexture(IBlockAccess ba, int x, int y, int z, int side) {
		...
		if (tile instanceof TilePumpPlus && side == 1) {
			if (((TilePumpPlus) tile).G_working()) return this.texW;
			if (((TilePumpPlus) tile).G_connected() != null) return this.texC;
		}
	...
	public void registerIcons(IconRegister par1IconRegister) {
		this.textureTop = par1IconRegister.registerIcon("qp2:pump_top");
		this.textureBottom = par1IconRegister.registerIcon("qp2:pump_bottom");
		this.textureSide = par1IconRegister.registerIcon("qp2:pump_side");
		this.texW = par1IconRegister.registerIcon("qp2:pump_top_w");
		this.texC = par1IconRegister.registerIcon("qp2:pump_top_c");
	}
	...
	public void breakBlock(World world, int x, int y, int z, int id, int meta) {
		this.drop.clear();
		TilePumpPlus tile = (TilePumpPlus) world.getBlockTileEntity(x, y, z);
	...
	public void onNeighborBlockChange(World w, int x, int y, int z, int bid) {
		((TilePumpPlus) w.getBlockTileEntity(x, y, z)).G_reinit();
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer ep, int side, float par7, float par8, float par9) {
		...
		if (equipped instanceof IToolWrench && ((IToolWrench) equipped).canWrench(ep, x, y, z)) {
			...
			((TilePumpPlus) world.getBlockTileEntity(x, y, z)).S_changeRange(ep);
			return true;
		}
		if (equipped instanceof ItemTool) {
			if (ep.getCurrentEquippedItem().getItemDamage() == 0) {
				if (world.isRemote) return true;
				for (String s : ((TilePumpPlus) world.getBlockTileEntity(x, y, z)).C_getNames())
			...
			if (ep.getCurrentEquippedItem().getItemDamage() == 2) {
				if (!world.isRemote) ((TilePumpPlus) world.getBlockTileEntity(x, y, z)).S_OpenGUI(side, ep);
				return true;
			}
		}
		return false;

...
public class BlockRefineryPlus extends BlockContainer {
	...
	public void breakBlock(World world, int x, int y, int z, int id, int meta) {
		this.drop.clear();
		TileRefineryPlus tile = (TileRefineryPlus) world.getBlockTileEntity(x, y, z);
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer ep, int side, float par7, float par8, float par9) {
		...
		if (equipped instanceof IToolWrench && ((IToolWrench) equipped).canWrench(ep, x, y, z)) {
			...
			if (liquid != null) {
				if (world.isRemote) return true;
				int qty = ((TileRefineryPlus) world.getBlockTileEntity(x, y, z)).fill(ForgeDirection.UNKNOWN, liquid, true);
	...
	public int getRenderType() {
		return QuarryPlus2.refineryRenderID;
	}

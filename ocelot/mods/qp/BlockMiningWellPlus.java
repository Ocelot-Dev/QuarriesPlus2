...
public class BlockMiningWellPlus extends BlockContainerBase {
	Icon textureFront, textureBack, textureTop, texW;

	public BlockMiningWellPlus(int par1) {
	...
	public TileEntity createNewTileEntity(World world) {
		return new TileMiningWellPlus();
	}

	@Override
	@SideOnly(Side.CLIENT)
	public void registerIcons(IconRegister par1IconRegister) {
		this.textureFront = par1IconRegister.registerIcon("qp2:miningwell_front");
		this.blockIcon = par1IconRegister.registerIcon("qp2:miningwell");
		this.textureBack = par1IconRegister.registerIcon("qp2:miningwell_back");
		this.textureTop = par1IconRegister.registerIcon("qp2:miningwell_top");
		this.texW = par1IconRegister.registerIcon("qp2:miningwell_top_w");
	}
	...
	public Icon getBlockTexture(IBlockAccess ba, int x, int y, int z, int side) {
		...
		if (tile instanceof TileMiningWellPlus && side == 1 && ((TileMiningWellPlus) tile).isActive()) return this.texW;
	...
	public void breakBlock(World world, int x, int y, int z, int id, int meta) {
		this.drop.clear();
		TileMiningWellPlus tile = (TileMiningWellPlus) world.getBlockTileEntity(x, y, z);
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer ep, int side, float par7, float par8, float par9) {
		...
		if (equipped instanceof IToolWrench && ((IToolWrench) equipped).canWrench(ep, x, y, z)) {
			((TileMiningWellPlus) world.getBlockTileEntity(x, y, z)).G_reinit();

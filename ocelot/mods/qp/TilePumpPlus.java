...
public class TilePumpPlus extends APacketTile implements IFluidHandler, IPowerReceptor, IEnchantableTile {
	...
	void C_recievePacket(byte pattern, ByteArrayDataInput data, EntityPlayer ep) {
		...
		case PacketHandler.StC_OPENGUI_MAPPING:// BI[Ljava.lang.String;
			...
			for (int i = 0; i < len; i++)
				this.mapping[target].add(data.readUTF());
			ep.openGui(QuarryPlus2.instance, QuarryPlus2.guiIdPump + target, this.worldObj, this.xCoord, this.yCoord, this.zCoord);
			break;
		}
	}
	...
	private Box S_getBox() {
		...
		if (tb instanceof TileQuarryPlus) return ((TileQuarryPlus) tb).box;
		return null;
	}

	void S_changeRange(EntityPlayer ep) {
		if (this.range >= (this.fortune + 1) * 2) {
			if (G_connected() instanceof TileQuarryPlus) this.quarryRange = true;
			this.range = 0;
